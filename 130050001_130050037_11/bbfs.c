...
int bb_getattr(const char *path, struct stat *statbuf)
{
    ...
    //==================
    statbuf->st_size=statbuf->st_size/2;
    //==================
    if (retstat != 0)
...
int bb_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi)
{
    ...
    log_fi(fi);
    //===========================
    retstat = pread(fi->fh, buf, 2*size, 2*offset);
    if (retstat < 0) {
    ...
    char xorkey='k';
    int offset_enc=0;
    int char_remaining=strlen(buf);
    unsigned char* intmp;
    unsigned char* outtmp;
    while(char_remaining>0){
      intmp=buf+2*offset_enc;
      outtmp=buf+offset_enc;
      if (intmp[0]==intmp[1])
        outtmp[0]=intmp[0]^xorkey;
      else{
          outtmp[0]=xorkey;
      }
      offset_enc+=1;
	     ...
	     struct fuse_file_info *fi)
{
    ...
    //===========================
    char *out = calloc(2*size, sizeof(char));
    char* in= calloc(size, sizeof(char));
    char xorkey='k';
    int offset_enc=0,char_remaining=size;
    strncpy(in,buf,size);
    unsigned char* intmp;
    unsigned char* outtmp;
    while(char_remaining>0){
        intmp=in+offset_enc;
        outtmp=out+2*offset_enc;
        if (intmp[0]!=xorkey){
            outtmp[0]=intmp[0]^xorkey;
            outtmp[1]=intmp[0]^xorkey;
        }
        else{
            int x=rand()%256;
            while(x<2){
    ...
    retstat = pwrite(fi->fh, out, 2*size, 2*offset);
